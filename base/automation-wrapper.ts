...
export class playwrightKeywords {
    ...
    protected async sendTextToElementInsideFrame(frameLocator: string, locator: string, text: string): Promise<void> {
        await this._page.frameLocator(frameLocator).locator(locator).fill(text);
    }
}